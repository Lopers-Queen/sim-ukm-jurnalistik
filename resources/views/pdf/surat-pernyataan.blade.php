...
<body>
    ...
    <div class="judul">
        ...
        <p>Nomor: {{ $surat->nomor_surat ?? '___/SP-UKM/___' }}</p>
    </div>
    ...
    <div class="isi">
        ...
        <div class="identitas">
            <table>
                ...
                <tr><td>NIM</td><td>: {{ $anggota->nim }}</td></tr>
                <tr><td>Program Studi</td><td>: {{ $anggota->program_studi ?? '-' }}</td></tr>
                <tr><td>Jurusan</td><td>: {{ $anggota->jurusan ?? '-' }}</td></tr>
                <tr><td>Status</td><td>: {{ ucfirst($anggota->status_keanggotaan) }}</td></tr>
            </table>
        </div>

        <div class="pernyataan">
            <p>Dengan ini menyatakan bahwa saya bersedia untuk berpartisipasi dalam kepanitiaan
               event <strong>"{{ $event->nama_event }}"</strong> yang akan dilaksanakan pada tanggal
               <strong>{{ $event->tanggal_mulai->translatedFormat('d F Y') }}</strong>
               {{ $event->tanggal_selesai ? 'sampai dengan ' . $event->tanggal_selesai->translatedFormat('d F Y') : '' }}
               di {{ $event->lokasi ?? '(lokasi akan ditentukan)' }}.</p>
    ...
    <div class="ttd-container">
        ...
        <div class="ttd-col">
            <p>Samarinda, {{ now()->translatedFormat('d F Y') }}<br>Yang Menyatakan,</p>
            <div class="ttd-space"></div>
            <p><strong>{{ $anggota->nama_lengkap }}</strong><br>NIM: {{ $anggota->nim }}</p>
        </div>
    </div>
</body>
</html>
